...
# 保存和上传新模型(未能下载，不更新上传模型，以便持续训练模型）
def save_and_upload_new_model(model, model_filename, commit_message):
    ...
    if latest_model_path:
        # 上传新模型到 GitHub
        upload_model_to_github(local_model_path, models_folder + model_filename, commit_message)
...
if uploaded_file is not None:
    ...
    # 上传新模型到 GitHub
    save_and_upload_new_model(local_model_path, f"{MODELS_DIR}MSD-{timestamp}.joblib",COMMIT_MSG_MODEL)
    st.write("模型已保存并上传到 GitHub。")
